                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                        <form class="d-flex" role="search">
                            <input :class="theme && 'nightInput'" class="form-control me-2" type="search"
                                placeholder="Search" aria-label="Search">
                            <button :class="theme && 'nightButtonSearch'" class="btn btn-outline-success me-4"
                                type="submit">Search</button>
                        </form>

                        <li class="nav-item">
                            <a :class="theme && 'nightButton'" class="nav-link active" aria-current="page"><router-link
                                    to="/" :class="theme && 'nightButton'">Home</router-link></a>
                        </li>
                        <li v-if="log" class="nav-item">
                            <a :class="theme && 'nightButton'" class="nav-link"><router-link
                                    :class="theme && 'nightButton'" to="/register">Register</router-link></a>
                        </li>
                        <li v-if="log" class="nav-item">
                            <a :class="theme && 'nightButton'" class="nav-link"><router-link
                                    :class="theme && 'nightButton'" to="/login">Login</router-link></a>
                        </li>
                        <li v-if="!log" class="nav-item">
                            <a :class="theme && 'nightButton'" class="nav-link"><router-link
                                    :to="{ name: 'EmployeeView', params: { Theme: theme } }"
                                    :class="theme && 'nightButton'">Employees</router-link></a>
                        </li>
                        <li class="nav-item dropdown">
                            <a :class="theme && 'nightButton'" class="nav-link dropdown-toggle" href="#" role="button"
                                data-bs-toggle="dropdown" aria-expanded="false" @click="checkUser">
                                {{ user }}
                            </a>
                            <ul class="dropdown-menu">
                                <!-- <li><a class="dropdown-item">{{profile}}</a></li> -->
                                <li><a class="dropdown-item"><router-link to="/profile">{{ profile }}</router-link></a>
                                </li>
                                <li><a class="dropdown-item">Billing</a></li>
                                <li>
                                    <hr class="dropdown-divider">
                                </li>
                                <li><a v-if="!log" class="dropdown-item logout-button" @click="logout">logout</a></li>
                            </ul>
                        </li>
                    </ul>
                </div>
